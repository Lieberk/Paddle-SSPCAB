...
class InferenceEngine(object):
    ...
    def __init__(self, args):
        ...
        self.predictor, self.config, self.input_tensor, self.output_tensor, self.density, self.threshold\
            = self.load_predictor(
    ...
    def load_predictor(self, model_file_path, params_file_path):
        ...
        with open(os.path.join(Path("eval") / f"model-{args.data_type}", "dict_train_embed.pkl"), "rb") as f:
            info = pickle.load(f)
            train_embed = paddle.to_tensor(info['train_embed'])
            threshold = paddle.to_tensor(info['threshold'])
        density.fit(train_embed)

        return predictor, config, input_tensor, output_tensor, density, threshold
...
def get_args(add_help=True):
    ...
    parser.add_argument("--img_path", default="images/good.png")
    parser.add_argument(
...
def infer_main(args):
    ...
    print(
        f"image_name: {args.img_path}, data is {'anomaly' if class_id == 1 else 'normal'}, score is {prob}, threshold is {inference_engine.threshold.numpy()[0]}")
    return class_id, prob
